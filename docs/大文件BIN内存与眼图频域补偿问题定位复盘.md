...
| 3000 万点、0～Nyquist、全频 2× 补偿 | 精确 `3N-2` 路径估算 12,995,072,032 B | 分块路径估算 543,883,464 B；最终补偿新增高水位 185,810,944 B | 真实 120,000,164 B Keysight BIN；输入输出均为 float32 |
...
包络提交依次为 `48ccd8e`、`eb31e84`；多通道后端余量修复提交为 `f03f256`。
...
| 阶段 | RSS 高水位/增量 |
|---|---:|
| 子进程基线 | 98,205,696 B，约 93.66 MiB |
| 加载后高水位 | 586,088,448 B，约 558.94 MiB |
| 加载新增峰值 | 487,882,752 B，约 465.28 MiB |
| 补偿后高水位 | 771,899,392 B，约 736.14 MiB |
| 已加载基础上的补偿新增峰值 | 185,810,944 B，约 177.20 MiB |
| 相对空进程的总峰值增量 | 673,693,696 B，约 642.48 MiB |

加载耗时约 0.198 s，补偿耗时约 0.598 s；相对独立闭式答案 `2*x` 的最大绝对误差为：
...
这张表只表示提交 `f03f256` 上一次 fresh worker 的 `ru_maxrss` 高水位样本；父进程生成
...
SHA-256 为 `c176865f79014b9b2652cabe72aa82dbb7a8cf608ceb5253099c3c42291fc367`。
