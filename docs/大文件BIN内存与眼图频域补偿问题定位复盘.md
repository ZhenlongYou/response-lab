...
| 3000 万点、0～Nyquist、全频 2× 补偿 | 精确 `3N-2` 路径估算 12,995,072,032 B | 分块路径估算 543,883,464 B；最终补偿新增高水位 250,462,208 B | 真实 120,000,164 B Keysight BIN；输入输出均为 float32 |
...
| 阶段 | RSS 高水位/增量 |
|---|---:|
| 子进程基线 | 98,385,920 B，约 93.83 MiB |
| 加载后高水位 | 580,665,344 B，约 553.77 MiB |
| 加载新增峰值 | 482,279,424 B，约 459.94 MiB |
| 补偿后高水位 | 831,127,552 B，约 792.62 MiB |
| 已加载基础上的补偿新增峰值 | 250,462,208 B，约 238.86 MiB |
| 相对空进程的总峰值增量 | 732,741,632 B，约 698.80 MiB |

加载耗时约 0.201 s，补偿耗时约 0.581 s；相对独立闭式答案 `2*x` 的最大绝对误差为：
...
`95,895,752 B`，约 21.4%。第一份原始 JSON 未改写地保存在
...
它们用于估算校准；后续 clean commit 上的 PASS 报告才是最终功能验收。

这张表只表示提交 `eb31e84` 上一次 fresh worker 的 `ru_maxrss` 高水位样本；父进程生成
...
SHA-256 为 `10428bdea6870bb161dd27da0d2e1be916df9642ba89ad025878d7a973b3cf9d`。
