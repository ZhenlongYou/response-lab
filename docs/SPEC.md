...
> 状态：当前代码的真实行为，更新于 2026-08-12。

## 1. 产品目标

ResponseLab 是一个面向 macOS 与 Windows 的桌面工具，用于比较参考拟合脉冲与待补偿拟合脉冲的复频响，
...
- 主“数据补偿”不按来源设置固定点数门限。CSV 在 `np.loadtxt` 前按文件体积、物理行数
  ...
  `Points` 按 `24 B/点 + 16 MiB` 在 payload 映射和时间轴分配前预检。GUI 还必须用同一
  已打开描述符的头部几何，在 payload 哈希/映射前执行完整补偿形状规划，并为即将常驻的
  float64 时间轴与 float32 幅值预留 `12 B/点`。两者与后续 DSP 使用相同动态预算。
  CSV/BIN 形成同一 `TimeSeries` 后，`run_compensation` 必须在响应分析、CZT、
...
- 默认从两份归一化脉冲谱共同不低于 -20 dB 的最长连续区间自动推荐谱宽候选；补偿边界取
  该区间的 2% 与 95%。线性相位拟合频带对当前文件组首次用该区间的 8% 与 90% 作为
  初值；同组后续自动补偿频带不再改写它。更换任一拟合脉冲时，未被用户手动编辑的相位
  拟合带必须按新文件重新建议，手动范围则保留并重新校验。数据补偿时补偿上限还受目标信号 Nyquist
...
默认边界合同为零延拓：小记录在首尾补零；大记录在精确路径超过安全预算时使用有限边界
重叠分块，块接缝取原记录真实相邻样本，只在完整记录两端补零。显式选择 `reflect` 时才
使用旧的全局镜像边界。工具通过 CZT 在实际 DFT 频点直接
...
- 一次只按 Vpp、眼高或眼宽中的一个指标排名。三类指标都使用两份等长的单通道拟合
  脉冲；DUT 相对参考脉冲的采样率差异最多允许 `1000 ppm`（`0.1%`），将门限内差异视为可共用
  离散网格，不执行重采样，超限时报告实际 ppm 并拒绝。Vpp 不再要求或读取参考/DUT
  原始采集波形。眼高/眼宽还要求
...
- Vpp 提供两种互斥方法：`LFP 峰峰值` 和 `频域 RMS 误差`。二者共享理想码型来源、
  ...
  0..4096 UI，默认峰前 3 UI、峰后 8 UI。界面初始 `M=32` 直接作为当前配置使用；用户修改
  后新值立即生效，更换拟合脉冲不会额外要求确认。
- 理想码型可选界面项 `PRBS13Q` 或单列外部文件。内置项固定为 8191-symbol Gray 映射周期，
...
- 候选标签表示用户所设的**满权核心**，界面可用 Hz、kHz、MHz 或 GHz 设置，默认
  100 MHz。每侧半余弦肩宽复用右栏“边缘过渡”百分比，默认是核心宽度的 10%；相邻核心
  的公共边界权重为 1，不留
  Tukey 零端点扫描缝。规则核心按所设宽度步进；若扫描跨度不能整除，最后追加一个完整
  核心并把其高端锚定到 `scan_high`。当有限
  脉冲或 Vpp 周期模型的物理分辨率更差时，核心向上扩为所设宽度的整数倍，肩宽
  同步按当前边缘过渡百分比计算，并给出告警。
- 幅度与幅相候选执行右栏当前最大补偿增益；即使主补偿模式为“仅相位”，影响分析仍
  比较三种候选，因此不会绕过用户保留的增益上限。眼高/眼宽有限记录还复用主补偿的
  `boundary_mode`，默认记录外补零；周期 Vpp 继续使用其明确的周期边界。
...
- 后台启动前用真实候选生成器估算成本：候选核心超过 2000 个或包含眼图
  ...
  目标
  样点数乘评估次数超过 5000 万时先提示长任务。取消在源文件分块哈希、CSV 逐行解析、
  BIN 头部/映射边界、成本估算后、工作区准备后、每个候选评估之间以及点选回放的前后
  检查；已进入的单次 FFT/IFFT 或
  卷积不强制终止，而是在该数值步骤完成后安全丢弃过期结果。
- 整包导出的响应诊断 CSV 必须分块生成，并在行块之间检查取消请求；不能为十列响应表
  一次性复制完整矩阵。输出批次已完整提交但临时文件或旧备份清理不完整时，GUI 必须
  显示警告和人工检查路径，不能只写入无控制台的 Python warning。
- 导出锁、目标指纹、原子提交和随机隔离名用于防止普通 ResponseLab 实例或良性并发覆盖
  已有输出；它不是针对同一操作系统账户内恶意进程的安全沙箱。同权限进程本身可直接
  改写用户目录，因而不承诺抵抗其在最后一次身份核验后定向抢占随机临时名。
