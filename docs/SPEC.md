...
- 两份拟合脉冲 CSV 与待补偿 CSV 共用三种明确合同：
  ...
  3. 兼容无表头 `time,value`，GUI 固定按秒和伏特解释，并要求每行恰好两个逻辑列；
     宽表只有底层 API 显式指定列时才允许。
...
- GUI 单 waveform 加载在 File Header 发现数量不为 1 时立即拒绝；通用头部检查最多保留
  4096 个 waveform 描述符和 4096 个全文件 buffer 描述符，超限时在构造更大对象图前停止。
...
- 主“数据补偿”不按来源设置固定点数门限。CSV 在 `np.loadtxt` 前按文件体积、物理行数
  和实际读取列估算解析峰值；GUI 的通用无表头与 Keysight/显式表头格式都解析完整两列，
  以发现任一数据行的多余列。底层 API 未启用严格合同时才可通过 `usecols` 显式选宽表列。BIN 根据头部
  `Points` 按 `24 B/点 + 16 MiB` 在 payload 映射和时间轴分配前预检。两者与后续 DSP
  ...
  N_FFT→2N_FFT 加密网格审计和 FFT 后端余量；安全预算为 `min(8 GiB, 50% 当前可用内存,
  当前可用内存 - 512 MiB)`，系统可用内存不可探测时使用 768 MiB 回退值。

## 3. 频响分析

每份脉冲分别计算 `dt · rfft(h[n])`，然后插值到从 0 到两者较小 Nyquist 的公共物理
频率轴。脉冲补零 FFT 长度必须为偶数且不小于请求长度，保证这个显示/诊断轴的 Nyquist
端点来自真实 RFFT bin；直接与分块内存估算必须使用同一长度函数。幅度差为：
...
小记录先做完整首尾镜像延拓；大记录在精确路径超过安全预算时使用有限边界重叠分块，
块接缝取原记录真实相邻样本，只在完整记录两端反射。工具通过 CZT 在实际 DFT 频点直接
计算两份拟合脉冲的有限记录频响，再按模式构造 `H_ref/H_dut`；显示分析网格不参与应用
响应的插值：
...
精确路径反变换后取回中间原记录；分块路径丢弃上下文区，只提交中央有效区。上下文由
补偿冲激响应的 float32 量化与截尾联合相对 L1 界决定；尾部会实际置零，量化已耗尽
预算或所需上下文过长时拒绝运行。输出与输入保持相同点数、
时间轴和通道数。
分块路径在此之前还要求脉冲有效时间支持满足 `2D<N_FFT`，并以 2N_FFT 直接 CZT 响应检查 N_FFT 点
循环冲激响应的离散网格收敛；相对 L∞ 超限即拒绝。该检查不宣称覆盖连续频率，且不与
量化/截尾的 L1 上界混为一个总界。
...
- Vpp 提供两种互斥方法：`LFP 峰峰值` 和 `频域 RMS 误差`。二者共享理想码型来源、
  ...
  0..4096 UI，默认各 8 UI。界面显示的初始 `M=32` 未确认时不能提交；用户修改或在
  字段内按 Enter 后才确认，任一拟合脉冲路径变化都会重新要求确认。
- 理想码型可选界面项 `内置 PRBS13Q Gray（8191）` 或单列外部文件。内置码型固定为 8191 symbols，
  Gray 符号码映射为归一化 PAM4 电平 `{-1,-1/3,+1/3,+1}`。GUI 的外部文件固定解释为
  整数符号码 `0..3`，不会按数值范围猜测或提供第二套“无量纲”选择。
...
- 输出预览的时域图只画记录开头连续 40960 点，避免长 BIN 让界面卡顿；完整补偿结果
  保留用于导出，不因预览裁剪。频谱仍使用有界连续窗口计算。
- 图例只命名数据对象，例如“补偿前”“补偿后”。抽样方式、窗口位置等实现细节不追加
  括号说明，避免在图内重复标注；只有确实区分不同物理数据时才增加必要限定。
...
`response-lab-manifest/v4` 记录输入来源、有效设置、线性相位斜率、带符号相对时延及其
