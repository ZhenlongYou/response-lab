...
- 一次只按 Vpp、眼高或眼宽中的一个指标排名。Vpp 使用参考和 DUT 原始波形；两条记录
  ...
  `样点数 = Np × M` 的拟合脉冲，并按用户选择的 NRZ 或 PAM4 固定符号序列回放。
- 眼指标先把固定符号写成间隔 `M` 点的冲激列，再与拟合脉冲做完整卷积；每条轨迹围绕
  冻结主光标截取 `-1 UI ~ +1 UI`，共 `2M+1` 点。参考主光标归一化为 1，DUT 与全部
  候选使用同一幅值尺度；DUT 候选还沿用补偿前冻结的主光标，不允许重新寻优或重定位。
- 默认候选标签表示 100 MHz **满权核心**。每侧再接 50 MHz 半余弦肩部，因此默认非零
  支撑宽度为 200 MHz；相邻核心的公共边界权重为 1，不留 Tukey 零端点扫描缝。规则
  核心按 100 MHz 步进；若扫描跨度不能整除，最后追加一个完整核心并把其高端锚定到
  `scan_high`。当有限脉冲或 Vpp 实际补偿目标记录的物理分辨率差于 100 MHz 时，核心
  向上扩为 100 MHz 的整数倍，肩宽同步保持为核心宽度的一半，并给出告警。
