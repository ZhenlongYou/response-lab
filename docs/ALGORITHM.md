...
对当前拟合脉冲组首次分析且用户尚未输入线性相位拟合频带时，用区间的 8% 与 90% 作为
初值。同组文件后续自动补偿频带只更新补偿边界，不改写相位拟合频带；同一窗口更换任一
拟合脉冲时，未被用户手动编辑的自动相位带会按新 Fs/Nyquist 重新建议，手动范围则保留
并由新输入重新校验。
...
默认把有限记录以外的样本声明为零；精确路径在首尾作零延拓，大记录在该路径超过预算时
使用有限边界重叠分块，块接缝读取原记录真实相邻点，只有完整记录两端补零。显式选择
`reflect` 才会恢复旧的全局镜像边界。两条路径都通过 CZT 在目标
...
主数据补偿不再给 BIN 单设固定点数硬上限。GUI 会在同一已打开 BIN 的头部扫描后、
payload 哈希/映射前按目标形状完成完整补偿规划，并为随后常驻的 float64 时间轴与
float32 幅值预留 `12 B/点`。CSV/BIN 进入同一 `TimeSeries` 后，
...
7. 分析 Vpp 时检查码型来源、当前 M 和 pmax 前后窗口；默认 M=32 可直接使用，修改后立即生效；分别核对 LFP 的 V 与频域误差的
...
9. 导出响应诊断表按固定行块写入临时文件，块间可取消；三份输出提交成功后若清理残留，
   窗口会给出需要人工检查的路径，而不是把清理警告隐藏在后台。
