...
首次分析且用户尚未输入去斜频带时，用区间的 8% 与 90% 作为初值并固定。后续自动
补偿频带只更新补偿边界，无论用户是否继续手动调整，都不会改写去斜频带。
...
“频率响应”绘图分别扣除参考与待补偿脉冲各自的线性相位，再给每个连续相位岛统一减去
整数个 `2π`，使残余相位可直接比较。“频响差异比较”同时显示去斜前与去斜后的相位；
响应 CSV 的 `phase_difference_deg` 保留原始展开相位差，
`phase_after_optional_detrend_deg` 记录按当前设置完成可选去斜并按岛归零的相位。界面不
单独输出时延结果。
