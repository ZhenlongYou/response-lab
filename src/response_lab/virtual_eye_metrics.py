...
def _measure_one_eye_width(
    ...
    # 最坏事件，因此沿用原来的五事件门槛。
    quantile_support = 1 if opening_probability == 0.0 else int(np.ceil(1.0 / opening_probability))
    ...
    supported = (left_counts >= minimum_crossing_count) & (right_counts >= minimum_crossing_count)
...
def _select_innermost_crossings_many(
    ...
    for batch_start in range(0, threshold_array.size, batch_size):
        ...
        shifted = (
            np.asarray(traces, dtype=float)[np.newaxis, :, :]
            - batch_thresholds[:, np.newaxis, np.newaxis]
        )
...
def _select_side_crossings_many(
    ...
    *,
    ...
    sign_changes = np.signbit(side_values[:, :, :-1]) != np.signbit(side_values[:, :, 1:])
    ...
    interpolated = (
        x_ui[start:stop][np.newaxis, np.newaxis, :]
        + fractions * np.diff(x_ui[start : stop + 1])[np.newaxis, np.newaxis, :]
    )
