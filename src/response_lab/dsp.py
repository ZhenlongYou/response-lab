...
def _all_finite_in_chunks(
    ...
    *,
    ...
    for start in range(0, flat.size, chunk_elements):
        raise_if_cancelled(cancelled, message="分析已取消")
        if not np.all(np.isfinite(flat[start : start + chunk_elements])):
            return False
    raise_if_cancelled(cancelled, message="分析已取消")
    return True
...
def _frequency_exceeds_upper_bound(value_hz: float, upper_hz: float) -> bool:
    ...
    tolerance_hz = max(abs(value_hz), abs(upper_hz), 1.0) * (32.0 * np.finfo(np.float64).eps)
...
def _validated_uniform_frequency_spacing(frequencies: FloatArray) -> float:
    ...
    for start in range(0, frequencies.size - 1, validation_chunk):
        ...
        if (
            np.any(intervals_hz <= 0.0)
            or float(np.max(np.abs(intervals_hz - spacing_hz))) > spacing_tolerance_hz
        ):
...
@dataclass(frozen=True)
class ResponseAnalysisMemoryEstimate:
    """脉冲比较/自动频带在首个 FFT 前使用的保守内存估算。"""

    reference_samples: int
    dut_samples: int
    analysis_points: int
    pulse_fft_samples: int
    pulse_fft_bins: int
    estimated_peak_bytes: int
...
@dataclass(frozen=True)
class CompensationMemoryPlan:
    """按目标形状完成门禁后得到的唯一补偿执行计划。"""

    strategy: Literal["exact", "streaming"]
    exact_estimate: CompensationMemoryEstimate | None
    streaming_estimate: StreamingCompensationMemoryEstimate | None
    available_memory_bytes: int | None
    anticipated_input_resident_bytes: int
    budget_bytes: int


@dataclass(frozen=True)
class PreparedResponseSpectra:
    """同一次请求内供自动建议与正式分析复用的两份脉冲频谱。"""

    reference_pulse: TimeSeries
    dut_pulse: TimeSeries
    analysis_points: int
    taper_alpha: float
    reference_spectrum: tuple[
        FloatArray,
        ComplexArray,
        FloatArray,
        NDArray[np.bool_],
...
def _response_analysis_shape_geometry(
    *,
    reference_samples: int,
    dut_samples: int,
    ...
    pulse_fft_samples = _next_fast_even_length(
        max(2 * longest_pulse, 2 * (int(settings.analysis_points) - 1))
    )
    pulse_fft_bins = pulse_fft_samples // 2 + 1
    ...
    return pulse_fft_samples, pulse_fft_bins, int(estimated_peak_bytes)
...
def _compensation_memory_estimate_from_shape(
    *,
    ...
    estimated_peak_bytes = (
        ...
        + samples * max(channels - 1, 0) * _COMPENSATION_EXTRA_CHANNEL_BYTES_PER_SAMPLE
...
def _streaming_memory_estimate_from_shape(
    *,
    ...
    refinement_low_index = max(
        0,
        int(np.floor(settings.band_low_hz * refinement_fft_samples / sample_rate_hz)),
    )
    refinement_high_index = min(
        refinement_rfft_bins - 1,
        int(np.ceil(settings.band_high_hz * refinement_fft_samples / sample_rate_hz)),
    )
    ...
    fixed_block_work_bytes = (
        ...
        + fft_samples * (np.dtype(np.float64).itemsize + 4 * np.dtype(np.int64).itemsize)
    )
    ...
    impulse_audit_bytes = fft_samples * _STREAMING_IMPULSE_AUDIT_BYTES_PER_FFT_SAMPLE
    ...
    backend_reserve_bytes = fft_samples * channels * _STREAMING_BACKEND_RESERVE_BYTES_PER_FFT_SAMPLE
    ...
    refinement_audit_bytes = (
        ...
        + refinement_czt_working_samples * _COMPENSATION_CZT_BYTES_PER_WORKING_SAMPLE
        + refinement_fft_samples * _STREAMING_BACKEND_RESERVE_BYTES_PER_FFT_SAMPLE
    )
    estimated_peak_bytes = (
        output_bytes
        + fixed_block_work_bytes
...
def _shape_preflight_budget(
    anticipated_input_resident_bytes: int,
) -> tuple[int | None, int]:
    """把尚未载入的输入常驻量纳入同一次系统内存快照。"""

    if (
        isinstance(anticipated_input_resident_bytes, (bool, np.bool_))
        or not isinstance(anticipated_input_resident_bytes, (int, np.integer))
        or int(anticipated_input_resident_bytes) < 0
    ):
        raise ValueError("预计输入常驻内存字节数必须是非负整数")
    resident_bytes = int(anticipated_input_resident_bytes)
    available = _system_available_memory_bytes()
    if available is None:
        budget = max(
            _safe_compensation_memory_budget_bytes(None) - resident_bytes,
            0,
        )
        return None, budget
    available_bytes = max(int(available), 0)
    remaining_after_input = max(available_bytes - resident_bytes, 0)
    return (
        available_bytes,
        _safe_compensation_memory_budget_bytes(remaining_after_input),
    )


def preflight_response_analysis_shape(
    *,
    reference_samples: int,
    dut_samples: int,
    settings: CompensationSettings,
    anticipated_input_resident_bytes: int = 0,
) -> ResponseAnalysisMemoryEstimate:
    """在自动建议、比较或正式分析的首个 FFT 前按形状执行门禁。"""

    pulse_fft_samples, pulse_fft_bins, estimated_peak_bytes = _response_analysis_shape_geometry(
        reference_samples=reference_samples,
        dut_samples=dut_samples,
        settings=settings,
    )
    available, budget = _shape_preflight_budget(
        anticipated_input_resident_bytes,
    )
    if estimated_peak_bytes > budget:
        available_text = _memory_preflight_context(
            available,
            int(anticipated_input_resident_bytes),
        )
        raise MemoryError(
            "响应分析内存预检拒绝启动：预计新增峰值工作区约 "
            f"{estimated_peak_bytes / (1024.0**2):.0f} MiB，"
            f"本次安全预算约 {budget / (1024.0**2):.0f} MiB{available_text}。"
            f"估算已计入 {pulse_fft_samples} 点脉冲 FFT、"
            f"{settings.analysis_points} 点公共分析网格及相位处理临时量；"
            "请缩短拟合脉冲或减少分析频点。"
        )
    return ResponseAnalysisMemoryEstimate(
        reference_samples=int(reference_samples),
        dut_samples=int(dut_samples),
        analysis_points=int(settings.analysis_points),
        pulse_fft_samples=pulse_fft_samples,
        pulse_fft_bins=pulse_fft_bins,
...
def _memory_preflight_context(
    available_memory_bytes: int | None,
    anticipated_input_resident_bytes: int,
) -> str:
    if available_memory_bytes is None:
        available_text = "，系统可用内存无法探测，已采用保守回退预算"
    else:
        available_text = f"，系统当前可用约 {available_memory_bytes / (1024.0**2):.0f} MiB"
    if anticipated_input_resident_bytes > 0:
        available_text += (
            "，其中已为尚未载入的输入保留约 "
            f"{anticipated_input_resident_bytes / (1024.0**2):.0f} MiB"
        )
    return available_text


def _reject_exact_memory_estimate(
    estimate: CompensationMemoryEstimate,
    *,
    available_memory_bytes: int | None,
    anticipated_input_resident_bytes: int,
    budget_bytes: int,
...
def _reject_streaming_memory_estimate(
    estimate: StreamingCompensationMemoryEstimate,
    *,
    available_memory_bytes: int | None,
    anticipated_input_resident_bytes: int,
    budget_bytes: int,
...
def preflight_compensation_shape(
    *,
    target_samples: int,
    target_channels: int,
    sample_rate_hz: float,
    ...
    """只按形状完成完整门禁，供载荷映射前与实际运行共同调用。

    ``anticipated_input_resident_bytes`` 用于 BIN 仍未映射的阶段。已加载目标调用方
    应传零，因为系统可用内存快照已经反映了该输入占用，不能重复扣减。
    """

    if not np.isfinite(sample_rate_hz) or sample_rate_hz <= 0.0:
        raise ValueError("待补偿信号采样率必须是正的有限值")
    target_nyquist_hz = float(sample_rate_hz) / 2.0
    if _frequency_exceeds_upper_bound(settings.band_high_hz, target_nyquist_hz):
    ...
    if settings.application_strategy == "streaming":
        streaming_estimate = _streaming_memory_estimate_from_shape(**common_shape)
        if streaming_estimate.estimated_peak_bytes > budget:
            _reject_streaming_memory_estimate(
                streaming_estimate,
                available_memory_bytes=available,
                anticipated_input_resident_bytes=resident_bytes,
                budget_bytes=budget,
            )
        return CompensationMemoryPlan(
            strategy="streaming",
            exact_estimate=None,
            streaming_estimate=streaming_estimate,
            available_memory_bytes=available,
            anticipated_input_resident_bytes=resident_bytes,
            budget_bytes=budget,
        )

    exact_estimate = _compensation_memory_estimate_from_shape(**common_shape)
    if exact_estimate.estimated_peak_bytes <= budget:
        return CompensationMemoryPlan(
            strategy="exact",
    ...
    if settings.application_strategy == "exact":
        _reject_exact_memory_estimate(
            exact_estimate,
            available_memory_bytes=available,
            anticipated_input_resident_bytes=resident_bytes,
            budget_bytes=budget,
...
def _interpolate_segments(
    ...
    for start, stop in _contiguous_runs(finite):
        ...
        target_mask = (target_frequency_hz >= source_x[0] - endpoint_tolerance_hz) & (
...
def prepare_response_spectra(
    reference_pulse: TimeSeries,
    dut_pulse: TimeSeries,
    settings: CompensationSettings,
    *,
    cancelled: CancellationCheck | None = None,
) -> PreparedResponseSpectra:
    """门禁后只计算一次两份脉冲频谱，供同一请求的建议与分析复用。"""
    ...
    reference_spectrum = _pulse_spectrum(
        reference_pulse,
        settings,
        cancelled=cancelled,
    )
    raise_if_cancelled(cancelled, message="分析已取消")
    dut_spectrum = _pulse_spectrum(
        dut_pulse,
        settings,
        cancelled=cancelled,
    )
    raise_if_cancelled(cancelled, message="分析已取消")
    for array in (*reference_spectrum, *dut_spectrum):
        array.setflags(write=False)
    return PreparedResponseSpectra(
        reference_pulse=reference_pulse,
        dut_pulse=dut_pulse,
        analysis_points=int(settings.analysis_points),
        taper_alpha=float(settings.taper_alpha),
        reference_spectrum=reference_spectrum,
        dut_spectrum=dut_spectrum,
    )


def _response_spectra_for_request(
    reference_pulse: TimeSeries,
    dut_pulse: TimeSeries,
    settings: CompensationSettings,
    prepared_spectra: PreparedResponseSpectra | None,
    *,
    ...
    if (
        prepared_spectra.reference_pulse is not reference_pulse
        or prepared_spectra.dut_pulse is not dut_pulse
        or prepared_spectra.analysis_points != int(settings.analysis_points)
        or prepared_spectra.taper_alpha != float(settings.taper_alpha)
    ):
        raise ValueError("预计算脉冲频谱与当前输入或分析设置不匹配")
    raise_if_cancelled(cancelled, message="分析已取消")
    return prepared_spectra.reference_spectrum, prepared_spectra.dut_spectrum
...
def suggest_frequency_settings(
    ...
    *,
    ...
    prepared_spectra: PreparedResponseSpectra | None = None,
    cancelled: CancellationCheck | None = None,
) -> CompensationSettings:
    ...
    reference_spectrum, dut_spectrum = _response_spectra_for_request(
        reference_pulse,
        dut_pulse,
        template,
        prepared_spectra,
    ...
    ref_f, _, ref_mag, _ = reference_spectrum
    dut_f, _, dut_mag, _ = dut_spectrum
    raise_if_cancelled(cancelled, message="分析已取消")
    ...
    candidates = [
        (start, stop)
        for start, stop in _contiguous_runs(eligible & (joint_db >= _AUTOMATIC_BAND_FLOOR_DB))
...
def _apply_compensation_safety(
    ...
    *,
    ...
    if transition_fraction == 0.0 or frequencies.size == 0:
        return applied
    transition_hz = transition_fraction * (settings.band_high_hz - settings.band_low_hz)
    ...
    nyquist_tolerance_hz = max(domain_high_hz, 1.0) * 32.0 * np.finfo(np.float64).eps
...
def _record_gain_limit_warning(
    ...
    warnings[:] = [
        warning for warning in warnings if not warning.startswith(_GAIN_LIMIT_WARNING_PREFIX)
    ]
...
def analyze_responses(
    ...
    *,
    ...
    ref_f, ref_h0, ref_mag, ref_reliable = reference_spectrum
    dut_f, dut_h0, dut_mag, dut_reliable = dut_spectrum
    raise_if_cancelled(cancelled, message="分析已取消")
    ...
    reliable = np.isfinite(ref_phase_common) & np.isfinite(dut_phase_common) & ref_valid & dut_valid
    band_mask = (frequency_hz >= settings.band_low_hz) & (frequency_hz <= settings.band_high_hz)
    if settings.mode in {"magnitude", "both"} and np.any(band_mask & ~dut_valid):
        raise ValueError("补偿频带内的待补偿脉冲响应为零，响应比无法计算；请缩小或移动补偿频带")
    if settings.mode in {"phase", "both"} and np.any(band_mask & ~reliable):
        raise ValueError("补偿频带内存在无法解析相位的频点；请缩小或移动补偿频带")
    ...
    if settings.mode == "magnitude":
        ...
        slope = fit_linear_phase_slope(frequency_hz, phase_difference, fit_weights, fit_mask)
    ...
    if settings.mode in {"magnitude", "both"}:
        log_amplitude[band_mask] = ref_log_common[band_mask] - dut_log_common[band_mask]
...
def _pulse_response_on_uniform_frequencies(
    ...
    *,
    ...
    direct_evaluation_error = horner_gamma * coefficient_l1 / pulse.sample_rate_hz
    ...
    if frequencies.size > 1:
        ...
        refine_mask = provisional_magnitude <= scale * _DIRECT_RESPONSE_REFINEMENT_RATIO
...
def _band_correction_for_rfft(
    ...
    *,
    ...
    while (
        ...
        and (high_index + 1) * frequency_step_hz <= analysis.settings.band_high_hz
    ):
        high_index += 1
    ...
    band_frequency_hz = np.arange(low_index, high_index + 1, dtype=np.float64) * frequency_step_hz
    reference_peak = 10.0 ** (float(np.max(analysis.reference_magnitude_db)) / 20.0)
    ...
    def project_real_endpoint(index: int, label: str) -> None:
        ...
        tolerance = max(abs(value), np.finfo(np.float64).tiny) * (_DIRECT_RESPONSE_REFINEMENT_RATIO)
...
def _record_block_with_boundary(
    array: np.ndarray,
    indices: NDArray[np.int64],
    boundary_mode: BoundaryMode,
) -> NDArray[np.float32]:
    """按声明的有限记录边界构造一个连续 float32 处理块。"""

    if boundary_mode == "reflect":
        mapped_indices = _reflect_record_indices(indices, array.shape[0])
        return np.ascontiguousarray(array[mapped_indices], dtype=np.float32)

    block = np.zeros((indices.size, array.shape[1]), dtype=np.float32)
    valid = (indices >= 0) & (indices < array.shape[0])
    block[valid] = array[indices[valid]]
    return block
...
def _validate_streaming_pulse_support(
    ...
    *,
    ...
    if 2.0 * support_samples >= fft_samples * (1.0 - 32.0 * np.finfo(np.float64).eps):
        raise ValueError(
...
def _streaming_refined_grid_relative_linf(
    ...
    *,
    ...
    mismatch_linf = max(
        mismatch_linf,
        float(np.max(np.abs(realized_response[refined_band_slice] - refined_band_correction))),
    )
...
def _apply_streaming_frequency_correction(
    ...
    *,
    ...
    raise_if_cancelled(cancelled, message="分析已取消")
    desired_impulse_l1 = float(np.sum(np.abs(desired_impulse64), dtype=np.longdouble))
    ...
    for start in range(0, fft_samples, quantization_chunk):
        ...
        difference = desired_impulse64[start:stop] - np.asarray(
            impulse32[start:stop], dtype=np.float64
        )
    ...
    if remaining_tail_l1 <= 0.0:
        raise ValueError(
            "float32 冲激响应量化误差已耗尽分块误差预算；请放宽分块尾部相对容差或改用整段精确路径"
        )
    quantized_impulse_l1 = float(np.sum(np.abs(impulse32), dtype=np.longdouble))
    ...
    if approximation_l1 > allowed_approximation_l1 * (1.0 + 32.0 * np.finfo(np.float64).eps):
    ...
    for start in range(0, samples, core_samples):
        ...
        block = _record_block_with_boundary(array, absolute_indices, boundary_mode)
    ...
    boundary_description = "零延拓" if boundary_mode == "zero" else "镜像延拓"
    warnings.append(
        f"大记录采用有限边界{boundary_description}与真实相邻样本重叠分块：主数据/FFT 使用 "
...
def apply_frequency_correction(
    ...
    *,
    ...
    信号先按 ``boundary_mode`` 在首尾各延拓一份记录，再在延拓记录的每个带内 DFT
    频点直接计算 ``H_ref/H_dut``，而不是从较粗的显示分析网格插值。最后取回中间
    原记录，避免把末端循环回卷到开头。
    """
    ...
    raise_if_cancelled(cancelled, message="分析已取消")
    pad_mode = "constant" if analysis.settings.boundary_mode == "zero" else "reflect"
    extended = np.pad(array, ((padding, padding), (0, 0)), mode=pad_mode)
    raise_if_cancelled(cancelled, message="分析已取消")
...
def compare_pulses(
    ...
    *,
    ...
    if settings.maximum_gain_db is not None and settings.mode in {"magnitude", "both"}:
        band = (analysis.frequency_hz >= settings.band_low_hz) & (
            analysis.frequency_hz <= settings.band_high_hz
        )
        requested_peak_db = (
            float(np.max(analysis.magnitude_difference_db[band])) if np.any(band) else float("-inf")
        )
...
def run_compensation(
    ...
    *,
    ...
    memory_plan = preflight_compensation_shape(
        ...
        dut_samples=dut_pulse.samples,
        settings=settings,
        anticipated_input_resident_bytes=0,
    )
    selected_strategy = memory_plan.strategy
    raise_if_cancelled(cancelled, message="分析已取消")
    ...
    if selected_strategy == "exact":
        exact_estimate = memory_plan.exact_estimate
        if exact_estimate is None:  # pragma: no cover - dataclass invariant
            raise RuntimeError("整段补偿内存计划缺少精确路径估算")
        ...
        application_method = (
            f"{settings.boundary_mode}_extend_czt_pulse_ratio_rfft_multiply_irfft_crop"
        )
        ...
        streaming_estimate = memory_plan.streaming_estimate
        if streaming_estimate is None:  # pragma: no cover - dataclass invariant
            raise RuntimeError("分块补偿内存计划缺少分块路径估算")
        ...
        application_method = f"finite_{settings.boundary_mode}_overlap_save_rfft_multiply_irfft"
        application_metadata = {
            ...
            "discarded_tail_relative_l1": (result.discarded_impulse_l1 / result.desired_impulse_l1),
            ...
            "impulse_approximation_relative_l1_bound": (
                (result.discarded_impulse_l1 + result.impulse_quantization_l1)
