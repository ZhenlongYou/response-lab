...
变化，不会把公共 Nyquist 当成可信带宽。去斜频带在首次分析时自动给出初值并固定；
无论用户是否继续手动调整，后续自动补偿频带都不会再改写该范围。

模式支持“仅幅频”“仅相频”“幅频 + 相频”。用户选择补偿频带；当前实现在频带边界
直接切换到单位响应，不会自动平滑过渡，因此过硬的频带边界可能带来时域振铃。
在含相位的模式中，工具会在用户指定的“去斜频带”内拟合相位差的线性项，并从相位差
中扣除该项，显示去斜前与去斜后的相位，便于比较残余相位。界面不把斜率另行换算成
时延结果。常数相位和非线性相位差仍会保留。“仅幅频”不执行相位去斜，但仍显示原始
相位差供观察。
...
“频率响应”页分别扣除两份脉冲各自的线性相位，再给每个连续相位岛统一平移整数个
`2π`，便于比较残余相位且不制造逐点 wrap 跳变。“频响差异比较”页显示去斜前与去斜
后的相位差；响应 CSV 同时保留二者。蓝色阴影表示分析或实际补偿频带，橙色虚线只标记
去斜频带边界，不代表额外补偿。
...
- `<输出文件>.response-lab.json`：输入文件哈希、有效参数、去斜斜率、频域应用信息、输出
  统计和输出文件哈希。
